...
bot.onContractEvent("on-transfer", contractAddress, (event: ContractEvent, state: any): Promise<any> => {
	if (event.eventName == "Transfer") {
		...
		bot.logInfo(`Transfer ${parsedValue} from ${payload.from} to ${payload.to} `);

		// Send the event...
		bot.emitEvent("ERC20-Transfer", payload);
	}
